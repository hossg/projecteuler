...
# substitutes the values in row N with the sum of the parent and the max of its children
def sumUpRow(rows, row):
    above = rows[row - 1]
    current = rows[row]
    for i, v in enumerate(above):
        above[i] = above[i] + max(current[i], current[i + 1])


# iterate over every row, from the bottom upwards, leaving the final value in the top row
def sumAllRows(rows):
    numRows = len(rows)
